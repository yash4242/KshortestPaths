...
int cmpfunc(const void *a, const void *b)
{
    journey *x = (journey *)a;
    journey *y = (journey *)b;
    return (10000 * (x->weight - y->weight));
}
...
void algo(int personIndex, int sourceCity, int destCity, int noOfCity, list graph[], city cityArray[], person personArray[], int noOfPerson)
{
    ...
    for (int i = 0; i <= noOfCity; i++)
        initQueue(&(queueOfCities[i]));

    insertInHeap(pq, sourceCity, sourceCity, 0.0, 0);

    //actual algo

    while (queueSize(&(queueOfCities[destCity])) <= 4)
    {
        ...
        popFromHeap(pq);
        queuePushBack(&(queueOfCities[top.b]), top.weight, top.a);
        ...
        {
            if (top.a == trav->b)
            { /*printf("was on %d, saw %d. rejecting edge from %d to %d\n", top.b, trav->b, trav->b, top.b);*/
                trav = trav->next;
                continue;
            }
    ...
    while (isQueueEmpty(&queueOfCities[destCity]) != 1 && pathsCovered < 3)
    {
        ...
        while (1) //no we need other breaking condition maybe?
        {
            ...
            //edgelength finding
            if (isQueueEmpty(&(queueOfCities[currCity])) == 1)
            {
            ...
            if (isQueueEmpty(&queueOfCities[currCity]) == 1)
            {
                printf("wouldve segfaulted on %d in algo.c\n", __LINE__);
                return;
            }
            ...
            if (isQueueEmpty(&queueOfCities[parentCity]) == 1)
            {
                printf("wouldve segfaulted on %d in algo.c parent city is %d\n", __LINE__, parentCity);
                return;
            }
            if (aim < queueFront(&queueOfCities[parentCity]).first - EPS)
            {
                queuePopFront(&queueOfCities[currCity]);
                printf("A path covered came back to city %d while already covering it earlier, so not much use.\n", parentCity);
        ...
        if (flag != 0) //new
            pushToStack(sp[pathsCovered], currCity);
        if (sp[pathsCovered]->arr[sp[pathsCovered]->topindex] != sourceCity)
        {
    ...
    if (isStackEmpty(sp[0]) == 1)
    {
        printf("stack is empty line %d in algo.c nothing would print", __LINE__);
    }
    ...
    while (isStackEmpty(s) != 1)
    {
        ...
        if (curr == nxt)
        {
            printf("source and destination are the same? returning back...\n");
            return;
        }
        if (transferPerson(personIndex, curr, nxt, personArray, cityArray, graph, noOfPerson) == 0)
        {
            return;
        }
        curr = nxt;
