...
int transferPerson(int personIndex, int sourceCity, int destCity, person personArray[], city cityArray[], list graph[], int noOfPerson)
{
    
    if(sourceCity == destCity){printf("source and destination are the same? returning back\n"); return 0;}
    ...
    if(trav == NULL)
    {
        printf("there is no edge bw %d and %d cities, returning to choice. dbg msg: quitting func %d line in city.c file\n",sourceCity, destCity, __LINE__);
        return 0;
    }

    if(personArray[personIndex].location != sourceCity)
    {
        printf("person %d doesnt live in city %d, quitting func. returning to choice. dbg msg:line %d in city.c file.\n", personIndex, sourceCity, __LINE__);
        return 0;
    }

    if(personArray[personIndex].covidStatus ==1)
    {
        printf("this person cant travel becuase he is infected, and thus in quarantine. returning to choice. dbg msg: line %d in city.c file, quitting func\n", __LINE__);
        return 0;
    }
    ...
    for(int i = 0; i < vip->numelems; i++)
    {
        ...
        if(trav == NULL)
        {
            printf("back to choice. error on line %d of city.c, node from %d to %d exists, but not in reverse? function quitting at iteration i = %d\n", __LINE__, sourceCity, vip->arr[i], i);
            return 0;
        }
